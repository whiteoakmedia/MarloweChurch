...
import { staffQuery } from "@/sanity/queries/index";
import { urlFor } from "@/sanity/image";
import { PortableText } from "@portabletext/react";
import type { Staff } from "@/sanity/queries/types";
...
export default async function OurLeadershipPage() {
  const sanityStaff = await safeFetch<Staff[]>(staffQuery);

  // Split into pastors and staff — if Sanity has data, use it; else fallback
  const useSanity = sanityStaff && sanityStaff.length > 0;

  // Staff with role containing "Pastor" are shown as pastors
  const pastors = useSanity
    ? sanityStaff.filter((s) => s.role?.toLowerCase().includes("pastor"))
    : null;
  const staffMembers = useSanity
    ? sanityStaff.filter((s) => !s.role?.toLowerCase().includes("pastor"))
    : null;
              ...
              >
                <div className="grid md:grid-cols-2 gap-0">
                  <div className="relative h-80 md:h-auto">
                    {leader.photo?.asset ? (
                      <Image
                        src={urlFor(leader.photo!).width(800).height(800).url()}
                        alt={leader.name}
                        fill
