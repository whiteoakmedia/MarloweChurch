...
import { sermonsQuery } from "@/sanity/queries/index";
import type { Sermon } from "@/sanity/queries/types";
...
export default async function SermonsPage() {
  const sermons = await safeFetch<Sermon[]>(sermonsQuery);
          ...
          {hasSermons ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {sermons.map((sermon) => {
                ...
                const thumbnailUrl = sermon.image?.asset
                  ? urlFor(sermon.image).width(640).height(360).url()
                  : youtubeId
