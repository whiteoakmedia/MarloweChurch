...
import { aboutPageQuery } from "@/sanity/queries/index";
import type { AboutPage, Belief } from "@/sanity/queries/types";
...
export default async function OurBeliefsPage() {
  const aboutPage = await safeFetch<AboutPage>(aboutPageQuery);
  const sanityBeliefs = aboutPage?.beliefs;
  // Use aboutPage.beliefs if available, otherwise fall back to hardcoded AG beliefs
  const beliefs: FallbackBelief[] = sanityBeliefs && sanityBeliefs.length > 0
    ? sanityBeliefs.map((b) => ({
        title: b.title,
        description: "", // PortableText rendering would go here if needed
      }))
    : fallbackBeliefs;
